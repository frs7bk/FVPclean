...
# إضافة context processor لإتاحة روابط التواصل الاجتماعي لجميع القوالب
@app.context_processor
def inject_social_media_links():
    """إتاحة روابط التواصل الاجتماعي لجميع القوالب"""
    try:
        return {'social_media_links': SocialMedia.query.filter_by(active=True).all()}
    except Exception as e:
        app.logger.error(f"خطأ في استدعاء روابط التواصل الاجتماعي: {str(e)}")
...
ALLOWED_EXTENSIONS = {'png', 'jpg', 'jpeg', 'gif', 'svg', 'pdf', 'webp', 'mp4', 'webm', 'ogg'}
...
def service_detail(service_type):
    ...
    # جلب جميع الخدمات الأخرى لعرضها في قسم "خدمات أخرى"
    all_services = []
    services_objects = Service.query.all()
    for s in services_objects:
        # تحويل كل كائن خدمة إلى قاموس لاستخدامه في القالب
        all_services.append({
            'title': s.title,
            'subtitle': s.subtitle,
...
def add_portfolio_item():
    ...
    try:
        ...
        # معالجة الفيديو (إذا وجد)
        video_url = None
        video_file = None
        video_thumbnail = None
        video_type = request.form.get('video_type', '')
        
        app.logger.info(f"Video type: {video_type}")
        
        if video_type == 'external':
            # معالجة رابط الفيديو الخارجي
            video_url = request.form.get('video_url', '').strip()
            app.logger.info(f"External video URL: {video_url}")
            
            # التحقق من وجود رابط
            if not video_url:
                app.logger.warning("External video URL is empty")
                flash('⚠️ تم تحديد نوع الفيديو كرابط خارجي ولكن لم يتم توفير الرابط', 'warning')
                video_type = ''  # إعادة تعيين نوع الفيديو لتجنب الإشارة إلى فيديو غير موجود
        
        elif video_type == 'local':
            # معالجة ملف الفيديو المحلي
            if 'video_file' in request.files:
                video_file_upload = request.files['video_file']
                if video_file_upload and video_file_upload.filename != '':
                    app.logger.info(f"Local video file: {video_file_upload.filename}")
                    
                    # إنشاء مجلد للفيديوهات إذا لم يكن موجوداً
                    videos_folder = os.path.join(projects_folder, 'videos')
                    videos_path = os.path.join(app.config['UPLOAD_FOLDER'], videos_folder)
                    os.makedirs(videos_path, exist_ok=True)
                    
                    # قائمة بامتدادات الفيديو المسموح بها
                    video_extensions = {'mp4', 'webm', 'ogg'}
                    file_ext = video_file_upload.filename.rsplit('.', 1)[1].lower() if '.' in video_file_upload.filename else ''
                    
                    # التحقق من حجم الملف (100 ميجابايت كحد أقصى)
                    max_video_size = 100 * 1024 * 1024  # 100 ميجابايت
                    
                    # قراءة الملف للحصول على حجمه
                    video_file_upload.seek(0, os.SEEK_END)
                    file_size = video_file_upload.tell()
                    video_file_upload.seek(0)  # إعادة تعيين مؤشر الملف
                    
                    if file_size > max_video_size:
                        app.logger.warning(f"Video file too large: {file_size} bytes (max: {max_video_size} bytes)")
                        flash(f'⚠️ حجم ملف الفيديو كبير جدًا. يجب أن يكون الحجم أقل من 100 ميجابايت.', 'warning')
                        video_type = ''  # إعادة تعيين نوع الفيديو في حالة فشل التحميل
                    elif file_ext in video_extensions:
                        try:
                            # إنشاء اسم فريد للملف لتجنب التضارب
                            import uuid
                            safe_filename = f"{uuid.uuid4().hex}.{file_ext}"
                            video_file = secure_file_save(video_file_upload, safe_filename, subfolder=videos_folder)
                            app.logger.info(f"Video file saved successfully as: {video_file}")
                        except Exception as e:
                            app.logger.error(f"Error saving video file: {str(e)}", exc_info=True)
                            flash(f'⚠️ حدث خطأ أثناء حفظ ملف الفيديو: {str(e)}', 'warning')
                            video_type = ''  # إعادة تعيين نوع الفيديو في حالة فشل التحميل
                    else:
                        app.logger.warning(f"Invalid video file type: {file_ext}")
                        flash(f'⚠️ نوع ملف الفيديو غير مدعوم. الأنواع المدعومة هي: mp4, webm, ogg', 'warning')
                        video_type = ''
                else:
                    app.logger.warning("Local video file is empty")
                    flash('⚠️ تم تحديد نوع الفيديو كملف محلي ولكن لم يتم توفير الملف', 'warning')
                    video_type = ''
        
        # معالجة الصورة المصغرة للفيديو (إذا وجدت)
        if video_type and 'video_thumbnail' in request.files:
            thumbnail_file = request.files['video_thumbnail']
            if thumbnail_file and thumbnail_file.filename != '':
                app.logger.info(f"Video thumbnail: {thumbnail_file.filename}")
                
                # التحقق من حجم الصورة المصغرة
                thumbnail_file.seek(0, os.SEEK_END)
                thumbnail_size = thumbnail_file.tell()
                thumbnail_file.seek(0)  # إعادة تعيين مؤشر الملف
                
                max_thumbnail_size = 5 * 1024 * 1024  # 5 ميجابايت
                
                if thumbnail_size > max_thumbnail_size:
                    app.logger.warning(f"Thumbnail file too large: {thumbnail_size} bytes")
                    flash(f'⚠️ حجم الصورة المصغرة كبير جدًا. يجب أن يكون الحجم أقل من 5 ميجابايت.', 'warning')
                elif allowed_file(thumbnail_file.filename):
                    try:
                        # حفظ الصورة المصغرة في مجلد المشاريع
                        thumbnail_folder = os.path.join(projects_folder, 'thumbnails')
                        thumbnail_path = os.path.join(app.config['UPLOAD_FOLDER'], thumbnail_folder)
                        os.makedirs(thumbnail_path, exist_ok=True)
                        
                        # إنشاء اسم فريد للصورة المصغرة لتجنب التضارب
                        file_ext = thumbnail_file.filename.rsplit('.', 1)[1].lower() if '.' in thumbnail_file.filename else ''
                        import uuid
                        safe_thumbnail_name = f"{uuid.uuid4().hex}.{file_ext}"
                        
                        video_thumbnail = secure_file_save(thumbnail_file, safe_thumbnail_name, subfolder=thumbnail_folder)
                        app.logger.info(f"Video thumbnail saved successfully as: {video_thumbnail}")
                    except Exception as e:
                        app.logger.error(f"Error saving video thumbnail: {str(e)}", exc_info=True)
                        flash(f'⚠️ حدث خطأ أثناء حفظ الصورة المصغرة للفيديو: {str(e)}', 'warning')
                else:
                    app.logger.warning(f"Invalid thumbnail file type: {thumbnail_file.filename}")
                    flash(f'⚠️ نوع ملف الصورة المصغرة غير مسموح به', 'warning')
...
def update_portfolio_item(portfolio_id):
    ...
    if request.method == 'POST':
        # تحديد الحد الأقصى لحجم الطلب (100 ميجابايت)
        max_content_length = 100 * 1024 * 1024  # 100 MB
        
        if request.content_length and request.content_length > max_content_length:
            app.logger.error(f"حجم الطلب كبير جدًا: {request.content_length} بايت (الحد الأقصى: {max_content_length} بايت)")
            flash("حجم الملفات المرفقة كبير جدًا. يجب أن يكون حجم الفيديو أقل من 100 ميجابايت.", "error")
            return redirect(url_for('admin_portfolio_management'))
    ...
    try:
        ...
        new_video_type = request.form.get('video_type', '')
        app.logger.info(f"Video type from form: {new_video_type}")
        
        # إذا تم اختيار فيديو خارجي، قم بتحديث الرابط
        if new_video_type == 'external':
            video_url = request.form.get('video_url', '')
            if video_url:
                app.logger.info(f"Updating external video URL: {video_url}")
                item.video_type = 'external'
                item.video_url = video_url
                # إعادة تعيين مسار الفيديو المحلي إذا كان موجوداً
                item.video_file = None
            else:
                app.logger.warning("External video URL is empty")
                flash('⚠️ تم تحديد نوع الفيديو كرابط خارجي ولكن لم يتم توفير الرابط', 'warning')
                item.video_type = item.video_type  # الاحتفاظ بالنوع الحالي
        
        # إذا تم اختيار فيديو محلي، تحقق من وجود ملف جديد
        elif new_video_type == 'local':
            app.logger.info(f"Local video type selected, checking for file upload")
            ...
            # الاحتفاظ بالفيديو القديم كوضع افتراضي
            keep_old_video = True
            
            # التحقق مما إذا تم رفع ملف جديد
            if 'video_file' in request.files and request.files['video_file'].filename != '':
                video_file_upload = request.files['video_file']
                app.logger.info(f"New local video file detected: {video_file_upload.filename}")
                
                # قائمة بامتدادات الفيديو المسموح بها
                video_extensions = {'mp4', 'webm', 'ogg'}
                file_ext = video_file_upload.filename.rsplit('.', 1)[1].lower() if '.' in video_file_upload.filename else ''
                
                # استخدام حجم الملف المتوفر بدون استخدام seek() و tell()
                # لتجنب مشاكل مع الملفات الكبيرة والذاكرة المؤقتة
                try:
                    # يمكن استخدام الحقل content_length إذا كان متاحًا
                    if hasattr(video_file_upload, 'content_length'):
                        file_size = video_file_upload.content_length
                    else:
                        # استخدام طريقة بديلة للحصول على حجم الملف دون استخدام seek/tell
                        video_file_upload.seek(0, os.SEEK_END)
                        file_size = video_file_upload.tell()
                        video_file_upload.seek(0)  # إعادة تعيين مؤشر الملف
                    ...
                    app.logger.warning(f"Could not determine video file size: {str(e)}")
                    file_size = 0
                ...
                if file_size > max_video_size:
                    ...
                    app.logger.info(f"Valid video file, proceeding to upload")
                    keep_old_video = False  # تم تقديم ملف جديد صالح، لا تحتفظ بالقديم
                    
                    try:
                        # حذف الفيديو القديم إذا كان موجوداً
                        if item.video_file and item.video_type == 'local':
                            old_video_filename = os.path.basename(item.video_file)
                            old_video_path = os.path.join(app.config['UPLOAD_FOLDER'], item.video_file)
                            if os.path.exists(old_video_path):
                                try:
                                    os.remove(old_video_path)
                                    app.logger.info(f"تم حذف ملف الفيديو القديم: {old_video_path}")
                                except Exception as e:
                                    app.logger.error(f"خطأ في حذف ملف الفيديو القديم: {str(e)}")
                        ...
                        item.video_type = 'local'
                        item.video_file = video_file
                        item.video_url = None  # إعادة تعيين الرابط الخارجي إذا كان موجوداً
                        
                    except Exception as e:
                        app.logger.error(f"Error saving video file: {str(e)}", exc_info=True)
                        flash(f'⚠️ حدث خطأ أثناء حفظ ملف الفيديو: {str(e)}', 'warning')
                        keep_old_video = True  # الاحتفاظ بالفيديو القديم في حالة الخطأ
                else:
                    app.logger.warning(f"Invalid video file type: {file_ext}")
                    flash(f'⚠️ نوع ملف الفيديو غير مدعوم. الأنواع المدعومة هي: mp4, webm, ogg', 'warning')
            
            # إذا لم يتم تحميل ملف جديد أو فشل التحميل، ولكن تم تحديد نوع "فيديو محلي"
            if keep_old_video:
                app.logger.info("No new video file uploaded, keeping existing settings if possible")
                
                # تعيين نوع الفيديو إلى محلي
                item.video_type = 'local'
                
                # إذا لم يكن هناك فيديو محلي سابق، عرض تنبيه
                if not item.video_file:
                    app.logger.warning("Local video type selected but no file provided and no existing video")
                    flash('⚠️ تم تحديد نوع الفيديو كملف محلي ولكن لم يتم توفير ملف ولا يوجد فيديو سابق', 'warning')
                else:
                    app.logger.info(f"Keeping existing local video: {item.video_file}")
                
                # إعادة تعيين الرابط الخارجي في جميع الحالات
                item.video_url = None
        
        # إذا تم اختيار بدون فيديو، قم بإعادة تعيين جميع معلومات الفيديو
        elif new_video_type == '':
            app.logger.info("Removing video information")
        ...
        # معالجة الصورة المصغرة للفيديو (إذا وجدت)
        if (new_video_type == 'local' or new_video_type == 'external') and 'video_thumbnail' in request.files:
            thumbnail_file = request.files['video_thumbnail']
            if thumbnail_file and thumbnail_file.filename != '':
                app.logger.info(f"Video thumbnail: {thumbnail_file.filename}")
                ...
                if thumbnail_size > max_thumbnail_size:
                    ...
                    try:
                        ...
                        if item.video_thumbnail:
                            app.logger.info(f"Deleting old video thumbnail: {item.video_thumbnail}")
                            old_thumbnail_path = os.path.join(app.config['UPLOAD_FOLDER'], item.video_thumbnail)
                            if os.path.exists(old_thumbnail_path):
                                try:
                                    os.remove(old_thumbnail_path)
                                    app.logger.info(f"تم حذف الصورة المصغرة القديمة: {old_thumbnail_path}")
                                except Exception as e:
                                    app.logger.error(f"خطأ في حذف الصورة المصغرة القديمة: {str(e)}")
                            else:
                                app.logger.warning(f"Old thumbnail not found at {old_thumbnail_path}")
                        ...
                        app.logger.info(f"Saving video thumbnail as: {safe_thumbnail_name} in folder: {thumbnail_folder}")
                        video_thumbnail = secure_file_save(thumbnail_file, safe_thumbnail_name, subfolder=thumbnail_folder)
                        app.logger.info(f"Video thumbnail saved successfully as: {video_thumbnail}")
                        
                        item.video_thumbnail = video_thumbnail
                    except Exception as e:
                        app.logger.error(f"Error saving video thumbnail: {str(e)}", exc_info=True)
                        flash(f'⚠️ حدث خطأ أثناء حفظ الصورة المصغرة للفيديو: {str(e)}', 'warning')
                else:
...
def delete_portfolio_item(portfolio_id):
    try:
        ...
        # حذف ملف الفيديو إذا كان موجوداً
        if portfolio_item.video_type == 'local' and portfolio_item.video_file:
            video_filename = os.path.basename(portfolio_item.video_file)
            videos_folder = os.path.join(projects_folder, 'videos')
            videos_path = os.path.join(app.config['UPLOAD_FOLDER'], videos_folder)
            ...
            if os.path.exists(video_path):
                try:
                    ...
                    app.logger.info(f"تم حذف ملف الفيديو: {video_path}")
                except Exception as e:
                    app.logger.error(f"خطأ في حذف ملف الفيديو: {str(e)}")
        
        # حذف الصورة المصغرة للفيديو إذا كانت موجودة
        if portfolio_item.video_thumbnail:
            thumbnail_filename = os.path.basename(portfolio_item.video_thumbnail)
            thumbnails_folder = os.path.join(projects_folder, 'thumbnails')
            thumbnails_path = os.path.join(app.config['UPLOAD_FOLDER'], thumbnails_folder)
            
            thumbnail_path = os.path.join(thumbnails_path, thumbnail_filename)
            if os.path.exists(thumbnail_path):
                try:
                    os.remove(thumbnail_path)
                    app.logger.info(f"تم حذف الصورة المصغرة للفيديو: {thumbnail_path}")
                except Exception as e:
                    app.logger.error(f"خطأ في حذف الصورة المصغرة للفيديو: {str(e)}")
