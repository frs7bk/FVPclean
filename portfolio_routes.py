...
def get_portfolio_categories():
    ...
    try:
        ...
        raw_categories = [cat[0] for cat in distinct_categories if cat[0]]
        
        # معالجة الفئات المتعددة (المفصولة بفواصل) واستخراج كل فئة على حدة
        categories = []
        for cat_string in raw_categories:
            # تقسيم السلسلة بناءً على الفاصلة وإزالة المسافات الزائدة
        ...
        # جلب سنوات المشاريع المتاحة
        years_query = db.session.query(
            db.func.extract('year', PortfolioItem.created_at).label('year')
        ).filter(PortfolioItem.created_at.isnot(None)).distinct().all()
        
        years = [int(year.year) for year in years_query if year.year]
        years.sort(reverse=True)  # ترتيب السنوات من الأحدث للأقدم
...
@portfolio_bp.route('/api/portfolio/filter/advanced', methods=['POST'])
def filter_portfolio_advanced():
    """واجهة برمجية للتصفية المتقدمة متعددة المعايير لمشاريع المحفظة"""
    try:
        data = request.json
        ...
        if category and category != "الكل":
            query = query.filter(
                db.or_(
                    ...
                    PortfolioItem.category.like(f"{category},%"),
                    PortfolioItem.category.like(f"%, {category},%"),
                    PortfolioItem.category.like(f"%, {category}")
                )
            )
        ...
        if year:
            try:
                year = int(year)
                query = query.filter(
                    db.func.extract('year', PortfolioItem.created_at) == year
                )
            except (ValueError, TypeError):
                # تجاهل فلتر السنة إذا كانت القيمة غير صالحة
                pass
        ...
        if content_type:
            if content_type == 'video':
                # مشاريع تحتوي على فيديو
                query = query.filter(
                    db.or_(
                        db.and_(PortfolioItem.video_url.isnot(None), PortfolioItem.video_url != ''),
                        db.and_(PortfolioItem.video_file.isnot(None), PortfolioItem.video_file != '')
                    )
                )
            elif content_type == 'carousel':
                # بما أن has_carousel هو property، نقوم أولا بجلب المشاريع ثم فلترتها
                all_results = query.all()
                carousel_items = [item for item in all_results if item.has_carousel]
                
                if not carousel_items:
                    # إذا لم تكن هناك نتائج، نعيد مصفوفة فارغة
                    return jsonify({
                        'success': True,
                        'items': [],
                        'count': 0
                ...
                # استخراج معرفات العناصر التي تحتوي على كاروسيل
                item_ids = [item.id for item in carousel_items]
                query = PortfolioItem.query.filter(PortfolioItem.id.in_(item_ids))
            elif content_type == 'image':
                # نبدأ بفلترة الفيديو (بما أن فلترته يمكن إجراؤها على مستوى قاعدة البيانات)
                no_video_query = query.filter(
                    db.or_(
                        PortfolioItem.video_url.is_(None),
                        PortfolioItem.video_url == ''
                    ),
                    db.or_(
                        PortfolioItem.video_file.is_(None), 
                        PortfolioItem.video_file == ''
                    )
                )
                
                # ثم نقوم بفلترة العناصر التي لا تحتوي على كاروسيل في الذاكرة
                items_without_video = no_video_query.all()
                image_only_items = [item for item in items_without_video if not item.has_carousel]
                ...
                # استخراج معرفات العناصر التي تحتوي على صور فقط
                item_ids = [item.id for item in image_only_items]
                query = PortfolioItem.query.filter(PortfolioItem.id.in_(item_ids))
        ...
        if sort_by:
            if sort_by == 'most-viewed':
                query = query.order_by(PortfolioItem.views_count.desc())
            elif sort_by == 'most-liked':
                query = query.order_by(PortfolioItem.likes_count.desc())
            elif sort_by == 'latest':
                query = query.order_by(PortfolioItem.created_at.desc())
            elif sort_by == 'oldest':
                query = query.order_by(PortfolioItem.created_at.asc())
        else:
        ...
        for item in portfolio_items:
            # تحضير البيانات لكل عنصر مع التعامل الآمن مع جميع القيم
            try:
                has_video = bool(item.video_url or item.video_file)
            except:
                has_video = False
                
            try:
                has_carousel = bool(item.has_carousel)
            except:
                has_carousel = False
            ...
            results.append({
                'id': item.id,
                'title': item.title or 'مشروع بدون عنوان',
                'description': item.description or '',
                'category': item.category or 'عام',
                'image_url': item.image_url or '/static/img/placeholder.jpg',
        ...
        app.logger.error(f"خطأ في التصفية المتقدمة للمحفظة: {str(e)}")
        return jsonify({
            'success': False,
            'message': str(e)
        }), 500
...
def filter_portfolio_by_category(category):
    ...
    try:
        ...
        if category == "الكل":
            ...
            # فلترة المشاريع حسب الفئة (البحث عنها كجزء من فئات متعددة مفصولة بفواصل)
            portfolio_items = PortfolioItem.query\
                .filter(
                    db.or_(
                        PortfolioItem.category == category,  # تطابق كامل
                        PortfolioItem.category.like(f"{category},%"),  # فئة في بداية السلسلة
                        PortfolioItem.category.like(f"%, {category},%"),  # فئة في وسط السلسلة
                        PortfolioItem.category.like(f"%, {category}")  # فئة في نهاية السلسلة
                    )
                )\
...
def get_portfolio_item_detail(portfolio_id):
    ...
    try:
        ...
        data = {
            ...
            'has_video': bool(portfolio_item.video_url or portfolio_item.video_file),
            'video_type': 'external' if portfolio_item.video_url else 'local',
            'video_url': portfolio_item.video_url or '',
            'video_file': portfolio_item.video_file or ''
        }
